...
int carrega_configuracio(FILE *fit)
{
  ...
  n_pil=i;
  if (ret!=4 && ret !=3 &&((n_fil!=0) || (n_col!=0)))			/* si no dimensions maximes */
  {
...
void * mou_pilota(void * index)
{
  ...
  {		/* si posicio hipotetica no coincideix amb la posicio actual */
	...
	if (c_pil != 0)		 		/* si ho surt del taulell, */
		win_escricar(f_pil,c_pil,*index,INVERS); /* imprimeix pilota */
	else
...
int main(int n_args, char *ll_args[])
{
  ...
  if (inicialitza_joc() !=0)	/* intenta crear el taulell de joc */
     ...
     for(i=0;i<n_pil;i++){
       pthread_create(&pilotes[i],NULL,mou_pilota,(void *) i);
     }
