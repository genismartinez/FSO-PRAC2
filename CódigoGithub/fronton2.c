...
#define LIMIT 9
int fi1, fi2, n_pil, fi3, tocs, cops;
			/* variables globals */
char *descripcio[]={
"\n",
...
int main(int n_args, char *ll_args[])
{
  ...
  if ((n_args!=3) && (n_args!=4))	/* si numero d'arguments incorrecte */
  { i=0;
  ...
  if (inicialitza_joc() !=0)	/* intenta crear el taulell de joc */
     ...
     do{
       ...
       sprintf(strin2,"Temps %i:%i  Tocs %i  Falten %i",minuts,segons, tocs, cops-tocs);
       win_escristr(strin2);
       win_retard(1000);
       segons++;
     }while(!fi1 && !fi2 && !fi3);
